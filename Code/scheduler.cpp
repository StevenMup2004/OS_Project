...
void print(int endtime, process processes[], int n)
{
    for (int j = 0; j < 4; j++) // no of processors
    {
        ...
        cout << output[endtime][j] <<  " | "<< endtime << "\n\n";
    }
    cout << "-----------------------processes------------------------\n";
    double avg = 0;
    for (int i = 0; i < n; i++)
    {
        cout << processes[i].process_name << " arrived at " << processes[i].arrive_time << " with burst time "<< processes[i].phases[0].first<< ". It's turn arround time = " << processes[i].turn_around_time << el;
        ...
        if (processes[i].state == 0) // state IO
        {
            ...
            if (processes[i].time_consumed == processes[i].current_brust_time) // IO Finished
            {
                ...
                else
                {
                    ...
                    processes[i].turn_around_time = t + 1 - processes[i].arrive_time;
                }
            }
        }
    }
...
void match_prefrences(int processor[], int WillGoToTheQ[], process processes[])
{
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      //      cout << "Process " << processes[WillGoToTheQ[i]].process_name
        //         << " returned to its previous processor " << processes[WillGoToTheQ[i]].last_processor << endl;
            WillGoToTheQ[i] = -1;
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        if (WillGoToTheQ[i] != -1)
        {
          ...
          //          cout << "Process " << processes[WillGoToTheQ[i]].process_name
            //             << " assigned to available processor " << j << endl;
                    WillGoToTheQ[i] = -1;
       ...
       // Log the final state of processors
  //*  cout << "Current processor states: ";
   // for (int i = 0; i < 4; i++)
    //{
      //  if (processor[i] == -1)
        //    cout << "Processor " << i << ": idle | ";
        //else
          //  cout << "Processor " << i << ": " << processes[processor[i]].process_name << " | ";
    //}
    //cout << endl;
