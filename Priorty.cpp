    ...
    for (int t = 0;; t++)
    {
        ...
        for (idx_p = 0; idx_p < 4; idx_p++)
        {
            ...
            if (processes[pr].time_consumed == processes[pr].current_brust_time) // proceses finished exec
            {
                ...
                {
                    processes[pr].state = processes[pr].phases[processes[pr].phase_idx].second; // In this case we use only 1 phase
                    processes[pr].current_brust_time = processes[pr].phases[processes[pr].phase_idx].first;// In this case we use only 1 phase
                }
