    ...
    for (int t = 0;; ++t)
    {
            ...
            if (queues_empty) {
                for (int idx_p = 0; idx_p < 4; idx_p++) {
                    if (processor[idx_p] == -1) {
                        ...
                        // cout << "Process " << processes[i].process_name << " added to processor " << idx_p +1  << endl;
                        break;
                    }
                }
            }
            ...
            if (!process_assigned) {
                for (int idx_p = 0; idx_p < 4; idx_p++) {
                    if (processor[idx_p] != -1 && processes[processor[idx_p]].priority > 0) {
                        my_queue[processes[processor[idx_p]].priority].push_front(processor[idx_p]);
                        processor[idx_p] = i;
                        processes[i].last_processor = idx_p;
                        process_assigned = true;
                        // cout << "Process " << processes[i].process_name << " preempted processor " << idx_p +1 << endl;
                        break;
                    }
                }
            }
        ...
        for (int idx_p = 0; idx_p < 4; ++idx_p)
        {
            ...
            if (processes[p].qunta % (1 << (processes[p].priority + 1)) == 0 || processes[p].time_consumed == processes[p].current_brust_time)
            {
                ...
                {
                    my_queue[processes[p].priority].push_back(p);
                }
        ...
        for (int idx_p = 0; idx_p < 4; ++idx_p)
        {
            ...
            if (current_queue < MAX_Q && !my_queue[current_queue].empty())
            {
                ...
                my_queue[current_queue].pop_front();
            }
        }
